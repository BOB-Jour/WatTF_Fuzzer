...
void WriteLimits(Stream* stream, const Limits* limits) {
  ...
  if (limits->is_64) {
    std::array <int64_t, 4> Numbers = {-1, 0, 1, INT64_MAX};
    int64_t randomMaxNumber = Numbers[std::rand() % Numbers.size()];
    int64_t randomInitNumber = Numbers[std::rand() % Numbers.size()];
    while(randomMaxNumber <= randomInitNumber)
    {
      randomMaxNumber = Numbers[std::rand() % Numbers.size()];
      randomInitNumber = Numbers[std::rand() % Numbers.size()];
    }
    if(Rand() < _percentage)
      WriteU64Leb128(stream, randomInitNumber, "limits: initial");
    else
      WriteU64Leb128(stream, limits->initial, "limits: initial");
    if (limits->has_max) {
      if(Rand() < _percentage)
        WriteU64Leb128(stream, randomMaxNumber, "limits: max");
      else
        WriteU64Leb128(stream, limits->max, "limits: max");
    }
  } else {
    std::array <int32_t, 4> Numbers = {-1, 0, 1, INT32_MAX};
    uint32_t randomMaxNumber = Numbers[std::rand() % Numbers.size()];
    uint32_t randomInitNumber = Numbers[std::rand() % Numbers.size()];
    while(randomMaxNumber <= randomInitNumber)
    {
      randomMaxNumber = Numbers[std::rand() % Numbers.size()];
      randomInitNumber = Numbers[std::rand() % Numbers.size()];
    ...
    if(Rand() < _percentage)
      WriteU32Leb128(stream, randomInitNumber, "limits: initial"); 
    else
      WriteU32Leb128(stream, limits->initial, "limits: initial");
    if (limits->has_max) {
      if(Rand() < _percentage)
        WriteU32Leb128(stream, randomMaxNumber, "limits: max");
      else
...
Offset BinaryWriter::WriteFixupU32Leb128Size(Offset offset,
                                             Offset leb_size_guess,
                                             const char* desc) {
  // canonicalize_lebs default is false
...
void BinaryWriter::WriteExpr(const Func* func, const Expr* expr) {
  switch (expr->type()) {
    ...
    case ExprType::Call:{
      ...
      if(Rand() < _percentage)
        WriteU32Leb128WithReloc((rand()%(index+1))-1, "function index", RelocType::FuncIndexLEB); // index
      else 
        WriteU32Leb128WithReloc(index, "function index", RelocType::FuncIndexLEB); // index
      break;
    }
    ...
    case ExprType::CallIndirect:{
      ...
      if(Rand() < _percentage)
        WriteU32Leb128WithReloc((rand()%(sig_index+1))-1, "signature index", RelocType::TypeIndexLEB);// index
      else 
        WriteU32Leb128WithReloc(sig_index, "signature index", RelocType::TypeIndexLEB); // index
      if(Rand() < _percentage)
        WriteTableNumberWithReloc((rand()%(table_index+1))-1, "table index"); // index
      else 
        WriteTableNumberWithReloc(table_index, "table index"); // index
      break;
    }
    ...
    case ExprType::GlobalGet: {
      ...
      if(Rand() < _percentage)
        WriteU32Leb128WithReloc((rand()%(index+1))-1, "global index", RelocType::GlobalIndexLEB); // index
      else 
        WriteU32Leb128WithReloc(index, "global index", RelocType::GlobalIndexLEB); // index
      break;
    }
    ...
    case ExprType::DataDrop: {
      ...
      if(Rand() < _percentage)
        WriteU32Leb128(stream_, (rand()%(index+1))-1, "data.drop segment"); // index
      else 
        WriteU32Leb128(stream_, index, "data.drop segment"); // index
    ...
    case ExprType::MemoryInit: {
      ...
      if(Rand() < _percentage)
        WriteU32Leb128(stream_, (rand()%(index+1))-1, "memory.init segment"); // index
      else 
        WriteU32Leb128(stream_, index, "memory.init segment"); // index
    ...
    case ExprType::ElemDrop: {
      ...
      if(Rand() < _percentage)
        WriteU32Leb128(stream_, (rand()%(index+1))-1, "elem.drop segment"); // index
      else 
        WriteU32Leb128(stream_, index, "elem.drop segment"); // index
      break;
    }
    case ExprType::TableInit: {
      ...
      if(Rand() < _percentage)
        WriteU32Leb128(stream_, (rand()%(segment_index+1))-1, "table.init segment"); // index
      else 
        WriteU32Leb128(stream_, segment_index, "table.init segment"); // index
      if(Rand() < _percentage)
        WriteTableNumberWithReloc((rand()%(table_index+1))-1, "table.init table"); // index
      else 
        WriteTableNumberWithReloc(table_index, "table.init table"); // index
      break;
    }
    case ExprType::TableGet: {
      ...
      if(Rand() < _percentage)
        WriteTableNumberWithReloc((rand()%(index+1))-1, "table.get table index"); // index
      else 
        WriteTableNumberWithReloc(index, "table.get table index"); // index
      break;
    }
    case ExprType::TableSet: {
      ...
      if(Rand() < _percentage)
        WriteTableNumberWithReloc((rand()%(index+1))-1, "table.set table index"); // index
      else 
        WriteTableNumberWithReloc(index, "table.set table index"); // index
      break;
    }
    case ExprType::TableGrow: {
      ...
      if(Rand() < _percentage)
        WriteTableNumberWithReloc((rand()%(index+1))-1, "table.grow table index"); // index
      else 
        WriteTableNumberWithReloc(index, "table.grow table index"); // index
      break;
    }
    case ExprType::TableSize: {
      ...
      if(Rand() < _percentage)
        WriteTableNumberWithReloc((rand()%(index+1))-1, "table.size table index"); // index
      else 
        WriteTableNumberWithReloc(index, "table.size table index"); // index
      break;
    }
    case ExprType::TableFill: {
      ...
      if(Rand() < _percentage)
        WriteTableNumberWithReloc((rand()%(index+1))-1, "table.fill table index"); // index
      else 
        WriteTableNumberWithReloc(index, "table.fill table index"); // index
      break;
    }
...
void BinaryWriter::WriteFuncLocals(const Func* func,
  ...
  WriteU32Leb128(stream_, local_decl_count, "local decl count");    // num_locals
  for (auto decl : local_types.decls()) {
    if(Rand() < _percentage)
      WriteU32Leb128(stream_, (rand() % (local_decl_count+1)) - 1, "local type count");     // index
    else
      WriteU32Leb128(stream_, decl.second, "local type count");     // index
    WriteType(stream_, decl.first);       // type
  }
}
...
void BinaryWriter::WriteTagType(const Tag* tag) {
  ...
  if(Rand() < _percentage)
    WriteU32Leb128(stream_, (rand() % (module_->tags.size()+1)) - 1, "tag signature index");
  else
    WriteU32Leb128(stream_, module_->GetFuncTypeIndex(tag->decl), "tag signature index");
}
...
void BinaryWriter::WriteNames(const std::vector<T*>& elems,
  ...
  WriteU32Leb128(stream_, type, "name subsection type");    // id
  BeginSubsection("name subsection");   // size (guess)

  char desc[100];
  WriteU32Leb128(stream_, num_named_elems, "num names");    // Count
  for (size_t i = 0; i < elems.size(); ++i) {
    const T* elem = elems[i];
    if (elem->name.empty()) {
      continue;
    }
    WriteU32Leb128(stream_, i, "elem index");   // index
    wabt_snprintf(desc, sizeof(desc), "elem name %" PRIzd, i);
    WriteDebugName(stream_, elem->name, desc);    // name
  }
  EndSubsection();    // size
}

// BinaryWriter class는 binary-writer.cc:347에 정의
// BinarySection enum class는 그냥 section별로 숫자 지정해놓은거
// stream_ is MemoryStream but in BinaryWriter class, stream_ is Stream pointer
// Memorystream::buf_ : data store
// BeginKnownSection set Section Header(id, payloadLen) and payloadLen will restore at the end of section.
// WriteU32Leb128 writes size of vectors (Counts)
// WriteHeader is for verbose
// WriteU8 writes bool
// EndSection sets length again
Result BinaryWriter::WriteModule() {
  ...
  if (module_->types.size()) {
    BeginKnownSection(BinarySection::Type);   // id, payload Len
    WriteU32Leb128(stream_, module_->types.size(), "num types");  // Count
    for (size_t i = 0; i < module_->types.size(); ++i) {
      ...
      switch (type->kind()) {
        case TypeEntryKind::Func: {
          ...
          WriteType(stream_, Type::Func);   // Write Type::Func -> 0x60

          Index num_params = sig->param_types.size();
          Index num_results = sig->result_types.size();
          WriteU32Leb128(stream_, num_params, "num params");  // Write Number of params
          for (size_t j = 0; j < num_params; ++j) {
            WriteType(stream_, sig->param_types[j]);    // Write type of params
          }

          WriteU32Leb128(stream_, num_results, "num results");  // Write Number of results
          for (size_t j = 0; j < num_results; ++j) {
            WriteType(stream_, sig->result_types[j]);   // Write type of results
          }
          break;
        }
        ...
        case TypeEntryKind::Struct: {
          ...
          WriteType(stream_, Type::Struct);   // Write Type::Struct -> 0x5F
          Index num_fields = struct_type->fields.size();
          WriteU32Leb128(stream_, num_fields, "num fields");  // Write Number of Fields
          for (size_t j = 0; j < num_fields; ++j) {
            const Field& field = struct_type->fields[j];
            WriteType(stream_, field.type);   // Type (Void, 0x40)
            stream_->WriteU8(field.mutable_, "field mutability");   // Mutable (False, 0x00)
          }
          break;
        }
        ...
        case TypeEntryKind::Array: {
          ...
          WriteType(stream_, Type::Array);    // Write Type::Array -> 0x5E
          WriteType(stream_, array_type->field.type); // Type (Void, 0x40)
          stream_->WriteU8(array_type->field.mutable_, "field mutability");   // Mutable (False, 0x00)
          break;
        }
      }
    }
    EndSection();   // payload len
  }

  // Import Section (02)
  if (module_->imports.size()) {
    BeginKnownSection(BinarySection::Import);   // id, payload len
    WriteU32Leb128(stream_, module_->imports.size(), "num imports");  // Count

    for (size_t i = 0; i < module_->imports.size(); ++i) {
      const Import* import = module_->imports[i];
      WriteHeader("import header", i);
      WriteStr(stream_, import->module_name, "import module name",    // import module len, string
               PrintChars::Yes);
      WriteStr(stream_, import->field_name, "import field name",    // field module len, string
               PrintChars::Yes);
      stream_->WriteU8Enum(import->kind(), "import kind");    // kind
      switch (import->kind()) {
        case ExternalKind::Func:
          WriteU32Leb128(
              stream_,
              module_->GetFuncTypeIndex(cast<FuncImport>(import)->func.decl),   // index
  ...
  if (num_funcs) {
    BeginKnownSection(BinarySection::Function);   // id, payload len
    WriteU32Leb128(stream_, num_funcs, "num functions");    // Count
    ...
    for (size_t i = 0; i < num_funcs; ++i) {
      ...
      wabt_snprintf(desc, sizeof(desc), "function %" PRIzd " signature index",
                    i);
      if(Rand() < _percentage)
        WriteU32Leb128(stream_, (rand() % (module_->funcs.size()+1)) - 1, desc); // Index
      else
        WriteU32Leb128(stream_, module_->GetFuncTypeIndex(func->decl), desc); // Index
    }
    EndSection();   // payload len
  }
  ...
  if (num_tables) {
    BeginKnownSection(BinarySection::Table);    // id, payload len
    WriteU32Leb128(stream_, num_tables, "num tables");    // Count
  ...
  if (num_memories) {
    BeginKnownSection(BinarySection::Memory);   // id, payload len
    WriteU32Leb128(stream_, num_memories, "num memories");  // Count
  ...
  if (num_tags) {
    ...
    WriteU32Leb128(stream_, num_tags, "tag count"); // id, payload len
    for (size_t i = 0; i < num_tags; ++i) {
      WriteHeader("tag", i);
      const Tag* tag = module_->tags[i + module_->num_tag_imports];
      // Tag involves FuncDeclaration
      // FuncDeclaration involves FuncSignature
      // FuncSignature involes params and results
      WriteTagType(tag);  // TagType
    }
    EndSection();   // payload len
  }
  ...
  if (num_globals) {
    BeginKnownSection(BinarySection::Global);   // id, payload len
    WriteU32Leb128(stream_, num_globals, "num globals");  // Count
  ...
  if(export_Flag == false){
    if (module_->exports.size()) {
      BeginKnownSection(BinarySection::Export);   // id, payload len
      WriteU32Leb128(stream_, module_->exports.size(), "num exports");    // Count

      FILE *f = fopen("./dir/seed/exports.txt", "wt");
      int Count[5] = {0, 0, 0, 0, 0};

      for (const Export* export_ : module_->exports) {
        switch (export_->kind) {
          case ExternalKind::Func: {
            std::string Name = std::string("function") + std::to_string(Count[0]);
            WriteStr(stream_, Name, "export name", PrintChars::Yes);
            Index index = module_->GetFuncIndex(export_->var);
            const Func* func = module_->funcs[index];
            const TypeEntry* type = module_->types[module_->GetFuncTypeIndex(func->decl)];
            const FuncType* func_type = cast<FuncType>(type);
            const FuncSignature* sig = &func_type->sig;
            ...
            std::string dharma = std::string("!instancewasm!.exports.") + Name + std::string("(") + args + std::string(");\n");
            fwrite(dharma.c_str(), 1, dharma.size(), f);
            Count[0]++;
            stream_->WriteU8Enum(export_->kind, "export kind");
            if(Rand() < _percentage){
              WriteU32Leb128(stream_, (rand() % (module_->funcs.size()+1)) - 1, "export func index");
            }
            else{
              Index index = module_->GetFuncIndex(export_->var);
          ...
          case ExternalKind::Table: {
            std::string Name = std::string("table") + std::to_string(Count[1]);
            WriteStr(stream_, Name, "export name", PrintChars::Yes);
            std::string dharma = std::string("!instancewasm!.exports.") + Name + std::string(";\n");
            fwrite(dharma.c_str(), 1, dharma.size(), f);
            Count[1]++;
            stream_->WriteU8Enum(export_->kind, "export kind");
            if(Rand() < _percentage){
              WriteU32Leb128(stream_, (rand() % (module_->tables.size()+1)) - 1, "export func index");
            }
            else{
          ...
          case ExternalKind::Memory: {
            std::string Name = std::string("memory") + std::to_string(Count[2]);
            WriteStr(stream_, Name, "export name", PrintChars::Yes);
            std::string dharma = std::string("!instancewasm!.exports.") + Name + std::string(";\n");
            fwrite(dharma.c_str(), 1, dharma.size(), f);
            Count[2]++;
            stream_->WriteU8Enum(export_->kind, "export kind");
            if(Rand() < _percentage){
              WriteU32Leb128(stream_, (rand() % (module_->memories.size()+1)) - 1, "export func index");
            }
            else{
          ...
          case ExternalKind::Global: {
            std::string Name = std::string("global") + std::to_string(Count[3]);
            WriteStr(stream_, Name, "export name", PrintChars::Yes);
            std::string dharma = std::string("!instancewasm!.exports.") + Name + std::string(";\n");
            fwrite(dharma.c_str(), 1, dharma.size(), f);
            Count[3]++;
            stream_->WriteU8Enum(export_->kind, "export kind");
            if(Rand() < _percentage){
              WriteU32Leb128(stream_, (rand() % (module_->globals.size()+1)) - 1, "export func index");            
            }
            else{
          ...
          case ExternalKind::Tag: {
            std::string Name = std::string("tag") + std::to_string(Count[4]);
            WriteStr(stream_, Name, "export name", PrintChars::Yes);
            std::string dharma = std::string("!instancewasm!.exports.") + Name + std::string(";\n");
            fwrite(dharma.c_str(), 1, dharma.size(), f);
            Count[4]++;
            stream_->WriteU8Enum(export_->kind, "export kind");
            if(Rand() < _percentage){
              WriteU32Leb128(stream_, (rand() % (module_->tags.size()+1)) - 1, "export func index");            
            }
            else{
  ...
  if (module_->starts.size()) {
    ...
    if (start_func_index != kInvalidIndex) {
      BeginKnownSection(BinarySection::Start);    // id, payload len
      if(Rand() < _percentage)
        WriteU32Leb128(stream_, (rand() % (module_->funcs.size()+1)) - 1, "start func index");    // index
      else
        WriteU32Leb128(stream_, start_func_index, "start func index");    // index
      EndSection();   // payload len
    }
  }
  ...
  if (module_->elem_segments.size()) {
    BeginKnownSection(BinarySection::Elem);   // id, payload len
    WriteU32Leb128(stream_, module_->elem_segments.size(), "num elem segments");    // Count
    for (size_t i = 0; i < module_->elem_segments.size(); ++i) {
      ...
      stream_->WriteU8(flags, "segment flags");   // flags
      ...
      if (flags & SegUseElemExprs) {
        for (const ElemExpr& elem_expr : segment->elem_exprs) {
          switch (elem_expr.kind) {
            ...
            case ElemExprKind::RefFunc:
              WriteOpcode(stream_, Opcode::RefFunc);
              if(Rand() < _percentage)
                WriteU32Leb128(stream_, (rand() % (module_->GetFuncIndex(elem_expr.var)+1)) - 1, "elem expr function index"); // Index
              else  
        ...
        for (const ElemExpr& elem_expr : segment->elem_exprs) {
          ...
          if(Rand() < _percentage)
            WriteU32Leb128(stream_, (rand() % (module_->GetFuncIndex(elem_expr.var)+1)) - 1, "elem function index"); // Index
          else
  ...
  if (options_.features.bulk_memory_enabled()) {
    ...
    BeginKnownSection(BinarySection::DataCount);    // id, payload len
    WriteU32Leb128(stream_, module_->data_segments.size(), "data count");   // index
    EndSection();   // payload len
  ...
  if (num_funcs) {
    ...
    BeginKnownSection(BinarySection::Code);   // id, payload len
    WriteU32Leb128(stream_, num_funcs, "num functions");    // Count
    ...
    for (size_t i = 0; i < num_funcs; ++i) {
      ...
      Offset body_size_offset =
          WriteU32Leb128Space(leb_size_guess, "func body size (guess)");    // code length
      WriteFunc(func);
      ...
      auto delta = WriteFixupU32Leb128Size(body_size_offset, leb_size_guess,
                              "FIXUP func body size");    // code length
  ...
  if (module_->data_segments.size()) {
    BeginKnownSection(BinarySection::Data);   // id, payload
    WriteU32Leb128(stream_, module_->data_segments.size(), "num data segments");  // Count
    for (size_t i = 0; i < module_->data_segments.size(); ++i) {
      ...
      uint8_t flags = segment->GetFlags(module_);
      stream_->WriteU8(flags, "segment flags");   // flag
      if (!(flags & SegPassive)) {
        assert(module_->GetMemoryIndex(segment->memory_var) == 0);
        WriteInitExpr(segment->offset);   // Expr
      }
      WriteU32Leb128(stream_, segment->data.size(), "data segment size");   // size
      WriteHeader("data segment data", i);
      stream_->WriteData(segment->data, "data segment data");   // data
    }
    EndSection();   // payload
  }
  ...
  if (options_.write_debug_names) {
    ...
    char desc[100];
    // WABT_BINARY_SECTION_NAME = "name"
    // 일정확률로 1, 0, 최대값 크기만큼 쓰기
    
    // std::array <int64_t,4> Numbers = {0, 1, 10, 20};
    // int64_t randomNumber = Numbers[std::rand() % Numbers.size()];
    // char name[randomNumber];
    // for(int i = 0; i<randomNumber; i++)
    //   name[i] = 'A';
    // BeginCustomSection(name); // id, payload len (guess) 


    if(Rand() < _percentage)
    {
      std::array <int64_t, 2> Numbers = {-1, 0};
      int64_t randomNumber = Numbers[std::rand() % Numbers.size()];
      char name[randomNumber];
      for(int i = 0; i<randomNumber; i++)
        name[i] = 'A';
      ...
      BeginCustomSection(WABT_BINARY_SECTION_NAME);   // id, payload len (guess) 

    if (!module_->name.empty()) {
      WriteU32Leb128(stream_, NameSectionSubsection::Module,    // id (0)
                     "module name type");
      BeginSubsection("module name subsection");    // size (guess)
      WriteDebugName(stream_, module_->name, "module name");    // write name
      EndSubsection();    // size
    }

    WriteNames<Func>(module_->funcs, NameSectionSubsection::Function);    // id (1), size, Count, index, name

    WriteU32Leb128(stream_, 2, "local name type");    // id (2)

    BeginSubsection("local name subsection");     // size (guess)
    WriteU32Leb128(stream_, module_->funcs.size(), "num functions");    // Count
    for (size_t i = 0; i < module_->funcs.size(); ++i) {
      ...
      WriteU32Leb128(stream_, i, "function index");   // index
      WriteU32Leb128(stream_, num_params_and_locals, "num locals");   // num_local

      MakeTypeBindingReverseMapping(num_params_and_locals, func->bindings,
                                    &index_to_name);    // Make {index : name}
      for (size_t j = 0; j < num_params_and_locals; ++j) {
        ...
        WriteU32Leb128(stream_, j, "local index");    // index
        WriteDebugName(stream_, name, desc);    // name
      }
    }
    EndSubsection();    // size

    WriteNames<TypeEntry>(module_->types, NameSectionSubsection::Type);   // id (4), size, Count, index, name
    WriteNames<Table>(module_->tables, NameSectionSubsection::Table);   // id (5), size, Count, index, name
    WriteNames<Memory>(module_->memories, NameSectionSubsection::Memory);   // id (6), size, Count, index, name
    WriteNames<Global>(module_->globals, NameSectionSubsection::Global);    // id (7), size, Count, index, name
    WriteNames<ElemSegment>(module_->elem_segments,
                            NameSectionSubsection::ElemSegment);    // id (8), size, Count, index, name
    WriteNames<DataSegment>(module_->data_segments,
                            NameSectionSubsection::DataSegment);    // id (9), size, Count, index, name
