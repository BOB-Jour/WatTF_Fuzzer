...
static std::string default_name = std::string("mutate");
static std::string default_path = std::string("./dir/mutate/");
...
examples:
  ...
  # If you want mutate wat file, give the option -m
  $ wat2wasm test.wat -m 100
)";

static void ParseOptions(int argc, char* argv[]) {
  OptionParser parser("wat2wasm", s_description);

  parser.AddOption('m', "mutation", "Mutate", "A number of mutated wasm files",
                    [](const char* argument) { N = atoi(argument); });
...
int ProgramMain(int argc, char** argv) {
  ...
  // option을 parsing -> -m option 추가 구현 필요
  ParseOptions(argc, argv);

  // s_infile은 wat file
  // s_infile의 내용을 읽어 file_data에 저장
  // WastLexer pointer를 생성, buffer를 가리키고 있는 pointer와 filename이 들어있는 class
  ...
  // Module은 ir.h:1055에 정의되어있으며 개복잡
  // feature는 option의 일부분인듯? feature.h에 정의
  // ParseWatModule에서 본격적으로 parsing. ParseModule은 wast-parser.cc:3255에 정의
  Errors errors;
  ...
  // 성공했으면 validation 검증
  // ValidateModule은 validator.cc:1064에 있으며 validator.CheckModule을 호출
  ...
  // WriteBinaryModule은 binary-writer.cc:1668에 있으며 BinaryWriter::WriteModule을 호출
  // BinaryWriter::WriteModule은 binary-writer.cc:1251에 있음
  for(int i=0; i<N; i++){
    if (Succeeded(result)) {
