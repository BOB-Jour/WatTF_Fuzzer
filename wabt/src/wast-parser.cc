...
// tokens_는 WastParser::tokens_ 에 선언되어 있는 circular Array
// Token은 token.h:78에 정의되어있는 class
TokenType WastParser::Peek(size_t n) {
  while (tokens_.size() <= n) {
    // WastLexer::GetToken()은 wast-lexer.cc:54에 정의. 앞에 있는 하나의 token을 들고옴
...
// WastParser parser에 이미 lexer, errors, option이 저장되어있음
// out_module은 Module을 가리키는 객체
// PeekMatchLpar()는 wast-parser.cc:549에 정의
...
// lexer: wat파일 data가 담겨있는 buffer
// out_module: Module을 가리키는 포인터
// errors: Error 유무를 저장할 객체
// options: parsing option에 대한 정보를 저장하고 있는 feature pointer
// ParseModule에서 본격적으로 parsing. ParseModule은 wast-parser.cc:1008에 정의
